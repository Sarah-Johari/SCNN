...
// Date         : ...
// File         : tb_cnn_layer.sv
// Desc         : Testbench for cnn_layer module (unfolded, modular version).
//
//   1. Write uniform weights (all=1) to all OUT_CHANNEL × IN_CHANNEL BRAMs
//   2. Write biases (bias[oc] = oc+1)
//   3. Apply input spikes, wait for processing
//   4. Print outspk and activation
//
//   Parameters: X_FANIN=4, Y_FANIN=5, X_KERNEL=2, Y_KERNEL=3,
//               STRIDE=1, IN_CHANNEL=2, OUT_CHANNEL=4,
//               WT_INTEGER_PRECISION=2
...
module tb_cnn_layer;
    ...
    localparam OUT_CH_ADDR_START    = 5;
    ...
    wire [PRECISION-1:0]       activation [OUT_CHANNEL-1:0][FANOUT-1:0];
    wire [PRECISION-1:0]       act;
    ...
    initial begin
        $display("=== cnn_layer Test Start ===");
        $display("IN_CHANNEL=%0d, OUT_CHANNEL=%0d, FANOUT=%0d, MEM_SIZE=%0d",
                 IN_CHANNEL, OUT_CHANNEL, FANOUT, MEM_SIZE);
        ...
        // ── Write biases (bias[oc] = oc+1) ───────────────
        $display("\nWriting biases...");
        for (int oc = 0; oc < OUT_CHANNEL; oc++)
            write_bias(oc[OUT_CH_WIDTH-1:0], (oc + 1));
        $display("  Done. Biases = [1, 2, 3, 4]");

        // ── Apply input spikes ───────────────────────────
        $display("\nApplying input spikes...");
        inspk[0] = '0;
        inspk[1] = '0;
        inspk[0][0*Y_FANIN + 0] = 1'b1;  // ch0: spike at (0,0)
        inspk[0][1*Y_FANIN + 1] = 1'b1;  // ch0: spike at (1,1)
        inspk[1][2*Y_FANIN + 3] = 1'b1;  // ch1: spike at (2,3)
        $display("  ch0: spikes at (0,0) and (1,1)");
        $display("  ch1: spike at (2,3)");

        // ── Wait for processing + CDC ────────────────────
        repeat(10) @(posedge spkclk);

        // ── Print results ────────────────────────────────
        $display("\nResults:");
        for (int oc = 0; oc < OUT_CHANNEL; oc++) begin
            $display("  OC%0d: outspk=%b, activation[0..3]=[%0d, %0d, %0d, %0d]",
                     oc, outspk[oc],
                     activation[oc][0], activation[oc][1],
                     activation[oc][2], activation[oc][3]);
        end

        $display("\n=== cnn_layer Test Completed ===");
        $finish;
    end
